        ...
        <div class="max-w-7xl mx-auto bg-white rounded-3xl shadow-2xl overflow-hidden">
          <header class="bg-gradient-to-r from-red-600 to-orange-600 text-white py-12 px-8 text-center">
            ...
            <p class="text-xl opacity-95">
              Open data from Philippine government agencies, mapped and
              connected with Neo4j
            </p>
          </header>
          ...
          <div class="grid grid-cols-2 md:grid-cols-5 gap-4 p-8 bg-gray-50 border-b">
            <div class="bg-white p-6 rounded-xl text-center shadow-md">
              <div class="text-3xl font-bold text-purple-600">
                {data.regions.toLocaleString("en-US")}
              </div>
              <div class="text-gray-600 mt-2">Regions</div>
            </div>
            <div class="bg-white p-6 rounded-xl text-center shadow-md">
              <div class="text-3xl font-bold text-purple-600">
                {data.provinces.toLocaleString("en-US")}
              </div>
              <div class="text-gray-600 mt-2">Provinces</div>
            </div>
            <div class="bg-white p-6 rounded-xl text-center shadow-md">
              <div class="text-3xl font-bold text-purple-600">
                {data.cities.toLocaleString("en-US")}
              </div>
              <div class="text-gray-600 mt-2">Cities</div>
            </div>
            <div class="bg-white p-6 rounded-xl text-center shadow-md">
              <div class="text-3xl font-bold text-purple-600">
                {data.municipalities.toLocaleString("en-US")}
              </div>
              <div class="text-gray-600 mt-2">Municipalities</div>
            </div>
            <div class="bg-white p-6 rounded-xl text-center shadow-md">
              <div class="text-3xl font-bold text-purple-600">
                {data.barangays.toLocaleString("en-US")}
              </div>
          ...
          <div class="p-8 space-y-8">
            <section>
              <h2 class="text-2xl font-bold text-gray-700 mb-4 pb-2 border-b-2 border-purple-600">
                📋 List Endpoints
              </h2>
              <div class="grid gap-3">
                <ApiEndpoint
                  method="GET"
                  path="/api/regions"
                  description="List all regions in the Philippines"
                />
                <ApiEndpoint
                  method="GET"
                  path="/api/provinces"
                  description="List all provinces with their parent region"
                />
                <ApiEndpoint
                  method="GET"
                  path="/api/cities"
                  description="List all cities (HUC, ICC, CC) with province and region"
                />
                <ApiEndpoint
                  method="GET"
                  path="/api/municipalities"
                  description="List all municipalities with province and region"
                />
                <ApiEndpoint
                  method="GET"
                  path="/api/localities"
                  description="List all cities and municipalities combined with type field"
                />
                <ApiEndpoint
                  method="GET"
                  path="/api/barangays?limit=100&offset=0&sort=name"
                  description="List barangays (paginated and sortable by psgc_code, name, or population)"
                />
              </div>
            </section>
            ...
            <section>
              ...
              <div class="grid gap-3">
                <ApiEndpoint
                  ...
                  tryUrl={data.examples.region
                    ? `/api/regions/${data.examples.region}`
                    : undefined}
                />
                <ApiEndpoint
                  method="GET"
                  ...
                  tryUrl={data.examples.province
                    ? `/api/provinces/${data.examples.province}`
                    : undefined}
                />
                <ApiEndpoint
                  method="GET"
                  ...
                  tryUrl={data.examples.city
                    ? `/api/cities/${data.examples.city}`
                    : undefined}
                />
                <ApiEndpoint
                  method="GET"
                  ...
                  tryUrl={data.examples.barangay
                    ? `/api/barangays/${data.examples.barangay}`
                    : undefined}
                />
              </div>
            </section>
            ...
            <section>
              ...
              <div class="grid gap-3">
                ...
                <ApiEndpoint
                  ...
                  tryUrl={data.examples.barangay
                    ? `/api/hierarchy/${data.examples.barangay}`
                    : undefined}
                />
              </div>
            </section>
              ...
              <div class="bg-gray-50 p-4 rounded-lg">
                <dl class="space-y-3">
                  <SearchParam
                    name="q"
                    required={true}
                    description="Search query string to match against location names"
                  />
                  <SearchParam
                    name="limit"
                  ...
                  <SearchParam
                    name="sort"
                    description="Sort results by: psgc_code, name (default), or population (descending)"
                  />
                  <SearchParam
                    name="type"
                    description="Filter by location type: region, province, city, municipality, or barangay"
                  />
                </dl>
              </div>
            </section>
          ...
          <footer class="bg-gray-50 p-8 text-center text-gray-600">
            ...
            <p class="mt-2">
              <a
                href="https://github.com/wewillcraft/philippine-datasets"
                target="_blank"
                class="text-purple-600 hover:underline"
              >
                View on GitHub
...
function ApiEndpoint(
  { method, path, description, tryUrl }: {
    method: string;
    path: string;
    description: string;
    tryUrl?: string;
      ...
      <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
        <div class="flex-1 min-w-0">
          <div class="flex items-center flex-wrap">
            <span class="inline-block bg-green-500 text-white px-2 py-1 rounded text-xs font-bold mr-2">
              {method}
            </span>
...
function CodeExample({ origin }: { origin: string }) {
  const examples = [
    ...
    },
    {
      ...
      command: "curl",
      url: `${origin}/api/search?q=davao&limit=10`,
    },
    {
      comment: '# Search only cities containing "Manila", sorted by population',
      command: "curl",
      url: `${origin}/api/search?q=Manila&type=city&sort=population&limit=10`,
    },
    {
      comment:
        "# Get complete hierarchy for Barangay Commonwealth, Quezon City",
      command: "curl",
      url: `${origin}/api/hierarchy/1374024000`,
    },
    {
      comment: "# Get paginated barangays (page 2, 100 items per page)",
      command: "curl",
      url: `${origin}/api/barangays?limit=100&offset=100`,
    },
  ];
    ...
    <section>
      ...
      <div class="grid gap-4">
        <GlossaryCard title="Administrative Levels">
          <GlossaryTerm
            term="Region"
            definition="The highest administrative division in the Philippines, grouping provinces with geographical, cultural, and ethnological characteristics."
          />
          <GlossaryTerm
            term="Province"
            definition="The primary subdivision of regions, serving as the main unit for implementing national programs and policies."
          />
          <GlossaryTerm
            term="City"
            definition="A local government unit with its own charter passed by Congress. Cities have larger populations, higher income requirements (₱100M+ annually), and more autonomy than municipalities. They can be classified as Highly Urbanized (HUC), Independent Component (ICC), or Component Cities (CC) based on their relationship with provinces."
          />
          <GlossaryTerm
            term="Municipality"
            definition="The default local government unit created through provincial ordinances. Municipalities have smaller populations and lower income requirements (₱2.5M+ annually) compared to cities. They are always part of a province and residents vote for provincial officials. Most rural areas are organized as municipalities."
          />
          <GlossaryTerm
            term="Sub-Municipality"
          ...
          <GlossaryTerm
            term="Barangay"
            definition="The smallest administrative division in the Philippines and the basic unit of governance. Each barangay is headed by a Barangay Captain."
          />
        </GlossaryCard>

        <GlossaryCard title="City Classifications">
          <GlossaryTerm
            term="HUC - Highly Urbanized City"
            definition="Cities with a minimum population of 200,000 and annual income of ₱50 million. They are independent from provinces and have their own congressional districts."
          />
          <GlossaryTerm
            term="ICC - Independent Component City"
            definition="Component cities whose charters prohibit their residents from voting for provincial elective officials. They are independent from the province in certain aspects."
          />
          <GlossaryTerm
            term="CC - Component City"
            definition="Cities that are part of a province and whose residents can vote for provincial officials. They share tax revenues with the provincial government."
          />
        </GlossaryCard>

        <GlossaryCard title="Special Administrative Areas">
          <GlossaryTerm
            term="NCR - National Capital Region"
            definition="Also known as Metro Manila, it's the only region without provinces. Cities and municipalities report directly to the regional government."
          />
          <GlossaryTerm
            term="BARMM - Bangsamoro Autonomous Region"
            definition="An autonomous region with its own government system, created through the Bangsamoro Organic Law."
          />
          <GlossaryTerm
            term="CAR - Cordillera Administrative Region"
            definition="An administrative region created to administer the provinces with similar cultural and geographical characteristics in the Cordillera mountain range."
          />
        </GlossaryCard>

        <GlossaryCard title="PSGC Code Structure">
          <p class="text-gray-600 mb-2">
            The 10-digit PSGC code follows the format:{" "}
            <code class="bg-gray-800 text-gray-300 px-2 py-1 rounded">
              RR-PPP-LL-BBB
            </code>
          </p>
          <ul class="ml-6 list-disc text-gray-600 space-y-1">
            <li>
              <strong>RR</strong> - Region code (01-19)
            </li>
            <li>
              <strong>PPP</strong>{" "}
              - Province code (000 for regions, 001-999 for provinces)
            </li>
            <li>
              <strong>LL</strong>{" "}
              - Locality (Municipality/City) code (00 for provinces, 01-99 for
              cities/municipalities)
            </li>
            <li>
              <strong>BBB</strong>{" "}
              - Barangay code (000 for cities/municipalities, 001-999 for
              barangays)
            </li>
          </ul>
          <p class="text-gray-600 mt-2">
            ...
            represents Barangay 007 (Laog) in Locality 01 (Angat) of Province
            014 (Bulacan) in Region 03 (Central Luzon).
          </p>
        </GlossaryCard>
      </div>
    </section>
