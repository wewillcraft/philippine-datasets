...
router.get("/search", async (ctx) => {
  ...
  const limit = Math.floor(parseInt(ctx.request.url.searchParams.get("limit") || "100"));
  const offset = Math.floor(parseInt(ctx.request.url.searchParams.get("offset") || "0"));
  const sort = ctx.request.url.searchParams.get("sort") || "name"; // psgc_code, name, population
  const type = ctx.request.url.searchParams.get("type"); // region, province, city, municipality, barangay
  ...
  if (!validSorts.includes(sort)) {
    ctx.response.status = 400;
    ctx.response.body = { error: "Invalid sort parameter. Must be one of: psgc_code, name, population" };
    return;
  }
  ...
  try {
    ...
    if (type) {
      ...
      if (!validTypes.includes(typeFilter)) {
        ctx.response.status = 400;
        ctx.response.body = { error: "Invalid type parameter. Must be one of: region, province, city, municipality, barangay" };
        return;
      }
      ...
      // Get paginated results with type filter
      ...
      // No type filter - search all types
      const results = await runQuery(`
        CALL {
          MATCH (r:Region)
          WHERE toLower(r.name) CONTAINS toLower($query)
    ...
    ctx.response.status = 500;
    ctx.response.body = { error: "Search failed", details: error.message };
  }
});
